...
const outputRoots = [
  ...
  'packages/react/lib',
  'packages/react/esm',
  'packages/react-sandbox/lib',
  'packages/react-sandbox/esm',
  'packages/react-settings-form/lib',
  'packages/react-settings-form/esm',
  'formily/transformer/lib',
  'formily/transformer/esm',
  'formily/setters/lib',
  'formily/setters/esm',
  'formily/antd/lib',
  'formily/antd/esm',
...
function files(path) {
  try {
    return readdirSync(path).flatMap((entry) => {
      ...
      return extensions.has(target.slice(target.lastIndexOf('.')))
        ? [target]
        : []
    })
  } catch (err) {
...
if (violations.length) {
  console.error(
    'Deprecated React, Ant Design, or old package APIs found:\n' +
      violations.join('\n')
  )
  process.exitCode = 1
} else {
  console.log(
    `✅ ${
      emitted ? 'Emitted-code' : 'Static'
    } audit passed: no deprecated React, Ant Design, or old package APIs found.`
  )
}
